...
on clicked theObject
	...
	tell window "main"
		if theObject is equal to button "input" then
			...
			set scriptCommand to "\"/usr/bin/runenergyplus\" \""
	...
	tell open panel
		if inputFileButtonClicked then
			...
			set theDirectory to "/Applications/EnergyPlus-8-1-0/ExampleFiles" as string
...
on launched theObject
	buildWeatherFilePopUp()
end launched

on buildWeatherFilePopUp()
	show window "main"
	set weather_files to {"-- none --"}
	set x to ((path to applications folder) as string) & "EnergyPlus-8-1-0:WeatherData"
